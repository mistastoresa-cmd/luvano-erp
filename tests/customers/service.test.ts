...
describe('CustomersService — CRUD', () => {
  ...
  it('updates a customer', async () => {
    ...
    const created = await customers.createCustomer({ tenantId: tenant.id, name: 'Ahmed Ali' })
    const updated = await customers.updateCustomer(tenant.id, created.id, { phone: '0559999999' })
  ...
  it('throws when updating a customer that does not belong to the tenant', async () => {
    ...
    await expect(
      customers.updateCustomer(otherTenant.id, created.id, { name: 'Hijacked' })
  ...
  it('gets a customer by id, scoped to tenant', async () => {
    ...
    expect(await customers.getCustomer(tenant.id, created.id)).not.toBeNull()
    expect(await customers.getCustomer(otherTenant.id, created.id)).toBeNull()
  })
  ...
  it('lists customers for a tenant only', async () => {
    ...
    await customers.createCustomer({ tenantId: tenant.id, name: 'Ahmed Ali' })
    await customers.createCustomer({ tenantId: tenant.id, name: 'Sara Mohammed' })
    await customers.createCustomer({ tenantId: otherTenant.id, name: 'Other Tenant Customer' })

    const list = await customers.listCustomers(tenant.id)
...
describe('CustomersService — interaction log', () => {
  it('logs and lists interactions for a customer, newest first', async () => {
    ...
    const customer = await customers.createCustomer({ tenantId: tenant.id, name: 'Ahmed Ali' })

    await customers.logInteraction({
      tenantId: tenant.id,
    ...
    const interactions = await customers.listInteractions(tenant.id, customer.id)
  ...
  it('scopes interaction listing to the given customer only', async () => {
    ...
    const customerA = await customers.createCustomer({ tenantId: tenant.id, name: 'Ahmed Ali' })
    const customerB = await customers.createCustomer({ tenantId: tenant.id, name: 'Sara Mohammed' })
    ...
    const interactionsA = await customers.listInteractions(tenant.id, customerA.id)
