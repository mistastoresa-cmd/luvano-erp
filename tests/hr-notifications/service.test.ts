...
describe('NotificationsService', () => {
  it('issues a warning letter and lists it for the employee, newest first', async () => {
    ...
    await notifications.issueNotification(SYSTEM_CONTEXT, {
      tenantId: tenant.id,
    ...
    const list = await notifications.listEmployeeNotifications(SYSTEM_CONTEXT, tenant.id, employee.id)
  ...
  it('stores relatedAmount for a deduction notification', async () => {
    ...
    const notification = await notifications.issueNotification(SYSTEM_CONTEXT, {
      tenantId: tenant.id,
      employeeId: employee.id,
      type: 'deduction',
      subject: 'خصم غياب',
  ...
  it('acknowledges a notification and rejects acknowledging it twice', async () => {
    ...
    const acknowledged = await notifications.acknowledgeNotification(SYSTEM_CONTEXT, tenant.id, notification.id)
    expect(acknowledged.status).toBe('acknowledged')
    expect(acknowledged.acknowledgedAt).not.toBeNull()

    await expect(notifications.acknowledgeNotification(SYSTEM_CONTEXT, tenant.id, notification.id)).rejects.toThrow(
      'not found or already acknowledged'
    )
  })
})

describe('NotificationsService — RBAC', () => {
  it('rejects staff issuing an official notification (owner/accountant/branch_manager only)', async () => {
    const db = await createTestDb()
    const { tenant } = await seedTenantWithBranch(db)
    const employee = await seedEmployee(db, tenant.id)
    const notifications = createNotificationsService(db)
    ...
    await expect(
      notifications.issueNotification(staff, {
        tenantId: tenant.id,
        employeeId: employee.id,
        type: 'warning',
        subject: 'إنذار',
